...
class MainPrePro:
    def prevals(self):
        lattices = ['Honeycomb', 'Square', 'Triangle', '3.6.3.6', '3.12.12', '3.3.3.4.4', '3.4.6.4', '4.6.12', '4.8.8']
    ...
    def nodes_widget(self, nodes, minp):
        ...
        for i in range(len(self.nodes)):
            if(i<len(self.nodes)):
                self.nitems[i].append(widgets.Dropdown(options = minp, value = i))
                for j in range(1, len(self.nodetext)-1):
    ...
    def elems_widget(self, ien):
        ...
        for i in range(len(self.ien)):
            if(i<len(self.ien)):
                self.Citems[i].append(widgets.Dropdown(options = minp, value = i))
