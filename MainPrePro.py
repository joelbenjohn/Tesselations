...
class MainPrePro:
    ...
    def nodes_widget(self, nodes, minp):
        ...
        for i in range(3):
            if(i<len(self.nodes)):
                self.nitems[i].append(widgets.IntText(value = i+1, disabled = True))
                self.nitems[i].append(widgets.Dropdown(options = self.minp, value = i+1))
        ...
        self.noder0 = widgets.VBox([self.node[j] for j in range(3)])
    ...
    def elems_widget(self, ien):
        ...
        for i in range(4):
            if(i<len(self.ien)):
                self.Citems[i].append(widgets.Dropdown(options = self.minp, value = i+1))
        ...
        self.ien1r0 = widgets.VBox([self.ien1[j] for j in range(4)])
